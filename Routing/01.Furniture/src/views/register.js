...
export async function registerPage(cntx) {
    cntx.render(registerTemplate(onSubmit));
    ...
    async function onSubmit(ev) {
        ...
        if (email == '' || password == '' || repass == '') {
            return cntx.render(registerTemplate(onSubmit, 'All fields are required!', email == '', password == '', repass == ''));
        }
        if (password != repass) {
            return cntx.render(registerTemplate(onSubmit, 'Password and RePassword are different', false, true, true));
        }
